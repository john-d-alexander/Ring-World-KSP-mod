...
{
    public enum Biome { Ocean, Lake, River, Wetland, Grassland, Forest, Desert, Mountain, Snow, Scrith, Ruins, Rimwall }
    ...
    public sealed class TerrainGenerator
    {
        ...
        public TerrainSample Sample(double along,double across)
        {
            ...
            double hills=Noise(along,across,6000,23), detail=Noise(along,across,350,29);
            double ridge=1-Math.Abs(Noise(along,across,42000,31)*2-1);
            double h=70+600*hills+32*detail+Math.Pow(ridge,7)*2400*Math.Max(0,(broad-.45)*2);
            ...
            double bank=Math.Min(1,Math.Abs(riverX)/500);
            h=Mix(riverLevel-7,h,Smooth(bank));
            if(Math.Abs(riverX)<500) { water=riverLevel; biome=Biome.River; }
            double lake=Math.Sqrt(Math.Pow((px-Math.Floor(px)-.5)*basin/3600,2)+Math.Pow(yy/2600,2));
            ...
            foreach(var l in Landmarks)
            {
                ...
                if(l.Kind=="ocean") { h=Mix(-1200,h,Smooth(Math.Max(0,Math.Min(1,(d-.8)/.4))));water=0;biome=Biome.Ocean; }
